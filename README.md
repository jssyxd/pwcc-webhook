...
| Silent fallback policy | When manually invoked within an intended `Asia/Shanghai` slot (`00, 02, 05, 07, 11, 13, 30, 32, 35, 37, 41, 43`), only stations without a current-instance Webhook record newer than 120 minutes are polled. The current Vercel Hobby deployment intentionally registers **no automatic Cron**. |
...
Set the vendor-supplied authentication details in the Vercel project environment using the variables documented in `.env.example`; do not commit any credentials. `GET /api/webhooks/xweather` is the readiness/challenge endpoint, `/api/weather/poll` is the manual station-level fallback, and `/api/weather/health` exposes current-instance logs, alert state, source outcomes and silent stations. The current Vercel Hobby deployment registers **no automatic Cron**, because its plan allows only daily jobs.

> **Quota gate.** If a future external scheduler invokes all 12 intended slots per hour, it will make **8,640 full-station batch HTTP requests per 30-day month**. This stays below a 15,000-access allowance only if Xweather permits 41 subrequests in one batch and measures each batch as one acceptable access. Before setting `XWEATHER_BATCH_METERING_VERIFIED=1`, run one authenticated 41-station batch, record `X-Cost-*` headers, and confirm the console usage delta. If subrequests are measured separately, do not enable that schedule.
